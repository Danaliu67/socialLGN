...
class PairDataset:
    def __init__(self, graph, src="lastfm"):
        self.src = src
        try:
            self.data_set = pd.read_csv(f'./data/preprocessed/{src}/data_set.txt')
            self.n_user = self.data_set['user'].nunique()
            self.m_item = self.data_set['item'].nunique()
        except IOError:
            self.interactionNet, self.n_user, self.m_item = loadInteraction(src)
            self.data_set = self.interactionNet
            self.data_set.to_csv(f'./data/preprocessed/{src}/data_set.txt', index=False)

        self.userData = np.array(self.data_set['user'])
        self.uniqueUser = np.unique(self.data_set['user'])
        self.itemData = np.array(self.data_set['item'])
        self._dataSize = len(self.data_set)
        print(f"{self._dataSize} interactions for processing")
        print(f"Number of users: {self.n_user}\n Number of items: {self.m_item}")
        print(f"Number of Ratings: {self._dataSize}")
        print(f"{graph} Rating Density: {self._dataSize / self.n_user / self.m_item}")

        # build (users, items), bipartite graph
        self.interactionGraph = None
        self.UserItemNet = csr_matrix((np.ones(len(self.data_set)), (self.userData, self.itemData)),
...
class GraphDataset(PairDataset):
    ...
    def getInteractionGraph(self):
        if self.interactionGraph is None:
            ...
            self.interactionGraph = self.interactionGraph.coalesce().to(device)
...
class SocialGraphDataset(GraphDataset):
    def __init__(self,  src):
        ...
        logging.info(f"{src} Link Density: {len(self.friendNet) / self.n_user / self.n_user}")
    ...
    def getSocialGraph(self):
        if self.socialGraph is None:
            ...
            GPU = torch.cuda.is_available()
            device = torch.device('cuda' if GPU else "cpu")
            self.socialGraph = self.socialGraph.coalesce().to(device)
